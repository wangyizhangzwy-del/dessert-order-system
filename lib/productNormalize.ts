...
const DESSERT_ROLL_MARKERS = /抹茶|焙茶|玄米|黑芝麻|香梨|杏子|铁观音|话梅|凤梨|红薯|香葱/i;
...
/** 凤梨 / 话梅 / 铁观音 卷（词序无关）。 */
function isFengliHuameiTieguanyinRoll(name: string): boolean {
  if (!/卷/.test(name)) return false;
  return hasAllWords(name, ["凤梨", "话梅", "铁观音"]);
}

/**  savory 香葱卷 / 酱多多卷 / 肉松卷 家族（排除甜卷）。 */
function isSavoryRollFamily(name: string): boolean {
  if (!/卷/.test(name)) return false;
  if (DESSERT_ROLL_MARKERS.test(name) && !/香葱/.test(name)) {
    if (/铁观音|话梅|凤梨/.test(name)) return false;
  }
...
export function normalizeProductName(raw: string | undefined | null): string {
  ...
  // 6. 凤梨话梅铁观音卷
  if (isFengliHuameiTieguanyinRoll(name)) return "凤梨话梅铁观音卷";
